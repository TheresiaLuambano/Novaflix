/**
 * BurudaniKiganjani - Series Page
 * Handles TV series browsing with filters and grid view
 */
...
class SeriesPage {
    ...
    setupSearch() {
        ...
        if (searchSubmit && searchInput) {
            searchSubmit.addEventListener('click', () => {
                ...
                if (query) {
                    window.location.href = `search.html?q=${encodeURIComponent(query)}&type=series`;
                }
            });
    ...
    async loadAllContent() {
        try {
            await Promise.all([
                ...
                this.loadBingeWorthy(),
                this.loadGenreSeries('action', 'actionRow'),
                this.loadGenreSeries('drama', 'dramaRow'),
                this.loadGenreSeries('comedy', 'comedyRow'),
                this.loadGenreSeries('anime', 'animeRow'),
                this.loadSeries()
            ]);
    ...
    async loadTrendingSeries() {
        ...
        try {
            const response = await api.getTrendingContent({ type: 'series', limit: 15 });
            if (response.status === 'success' && response.data && response.data.trending) {
                this.content.trending = response.data.trending;
                container.innerHTML = response.data.trending.map(item => this.createSeriesCard(item)).join('');
                this.attachCardListeners(container);
            } else {
                container.innerHTML = '<p class="empty-message">No trending series available</p>';
            }
        } catch (error) {
            console.error('Error loading trending series:', error);
            container.innerHTML = '<p class="empty-message">Failed to load trending series</p>';
        }
    }
    ...
    async loadNewReleases() {
        ...
        try {
            const response = await api.request('/content/new-releases.php?type=series&limit=15');
            if (response.status === 'success' && response.data && response.data.releases) {
                this.content.newReleases = response.data.releases;
                container.innerHTML = response.data.releases.map(item => this.createSeriesCard(item)).join('');
                this.attachCardListeners(container);
            } else {
            ...
            if (this.content.trending.length > 0) {
                container.innerHTML = this.content.trending.map(item => this.createSeriesCard(item)).join('');
                this.attachCardListeners(container);
            } else {
    ...
    async loadBingeWorthy() {
        const container = document.getElementById('bingeRow');
        if (!container) return;

        try {
            // Get top rated series for binge-worthy section
            const response = await api.getContentList({ type: 'series', sort: 'rating', limit: 15 });
            if (response.status === 'success' && response.data) {
                const series = Array.isArray(response.data) ? response.data : (response.data.content || []);
                this.content.bingeWorthy = series;
                container.innerHTML = series.map(item => this.createSeriesCard(item)).join('');
                this.attachCardListeners(container);
            } else {
                container.innerHTML = '<p class="empty-message">No binge-worthy series available</p>';
            }
        } catch (error) {
            console.error('Error loading binge-worthy:', error);
            container.innerHTML = '<p class="empty-message">Failed to load binge-worthy series</p>';
        }
    }

    async loadGenreSeries(genre, containerId) {
        const container = document.getElementById(containerId);
        if (!container) return;

        try {
            const response = await api.getContentByGenre(genre, { type: 'series', limit: 12 });
            if (response.status === 'success' && response.data && response.data.content) {
                this.content[genre] = response.data.content;
                container.innerHTML = response.data.content.map(item => this.createSeriesCard(item)).join('');
                this.attachCardListeners(container);
            } else {
                container.innerHTML = '<p class="empty-message">No series in this genre</p>';
            }
        } catch (error) {
            console.error(`Error loading ${genre} series:`, error);
    ...
    async loadSeries() {
        ...
        try {
            ...
            if (response.status === 'success' && response.data) {
                const series = Array.isArray(response.data) ? response.data : (response.data.content || []);
                this.content.all = series;
                
                // Update results count
                const total = response.pagination?.total || series.length;
                if (resultsCount) {
                    resultsCount.textContent = `${total} series${total !== 1 ? '' : ''}`;
                }
                ...
                container.innerHTML = series.map(item => this.createSeriesCard(item, 'grid')).join('');
                this.attachCardListeners(container);
                this.renderPagination(response.pagination);
            } else {
                Toast.error('Failed to load series');
    ...
    updateHero() {
        ...
        if (heroTitle) heroTitle.textContent = this.heroContent.title || 'TV Series';
        if (heroDescription) heroDescription.textContent = this.heroContent.description || 'Discover amazing series';
        ...
        if (heroDuration) heroDuration.textContent = this.heroContent.season_count 
            ? `${this.heroContent.season_count} Season${this.heroContent.season_count > 1 ? 's' : ''}` 
            : 'TV Series';
    }

    createSeriesCard(content, layout = 'row') {
        ...
        // Format seasons info
        const seasonsInfo = content.season_count 
            ? `${content.season_count} Season${content.season_count > 1 ? 's' : ''}`
        ...
        const cardHtml = `
            <div class="content-card" data-id="${content.id}">
                <div class="content-card-inner">
                    ...
                    ${content.is_new ? '<div class="new-badge">NEW</div>' : ''}
                </div>
                        ...
                        <div class="details-hero">
                            ...
                            <div class="details-hero-content">
                                ...
                                <div class="details-meta">
                                    ...
                                    <span class="duration">${seasonsInfo}</span>
                                </div>
                                <p class="description">${content.description || 'No description available.'}</p>
                                <div class="details-actions">
                                    <button class="btn btn-primary" onclick="seriesPage.playContent(${JSON.stringify(content).replace(/"/g, '"')})">
                                        <i class="fas fa-play"></i> Play Episode 1
                                    </button>
                                    <button class="btn btn-secondary" onclick="seriesPage.toggleFavorite(${content.id})">
        ...
        return `
            <div class="empty-state" style="grid-column: 1 / -1; padding: 3rem; text-align: center;">
                ...
                <p class="empty-state-description">
                    Try adjusting your filters or browse all series.
                </p>
                <button class="btn btn-primary" onclick="seriesPage.clearFilters()">
    ...
    renderPagination(pagination) {
        ...
        let html = `
            <div class="pagination">
                <button class="pagination-btn" ${current_page === 1 ? 'disabled' : ''} 
                        onclick="seriesPage.changePage(${current_page - 1})">
        ...
        if (startPage > 1) {
            html += `<button class="pagination-btn" onclick="seriesPage.changePage(1)">1</button>`;
            if (startPage > 2) {
        ...
        for (let i = startPage; i <= endPage; i++) {
            html += `
                <button class="pagination-btn ${i === current_page ? 'active' : ''}" 
                        onclick="seriesPage.changePage(${i})">${i}</button>
            `;
        }
        ...
        if (endPage < total_pages) {
            ...
            html += `<button class="pagination-btn" onclick="seriesPage.changePage(${total_pages})">${total_pages}</button>`;
        }
